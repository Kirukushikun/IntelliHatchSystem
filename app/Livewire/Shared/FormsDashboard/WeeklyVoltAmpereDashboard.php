...
class WeeklyVoltAmpereDashboard extends Component
{
    public int $typeId;
    ...
    {
        $this->formType = FormType::where('form_name', 'Weekly Voltage and Ampere Monitoring')->where('isActive', true)->first();

        if (! $this->formType) {
            abort(404, 'Weekly Volt/Ampere form type not found');
        }
    ...
    private function getFormPhotos(int $formId, ?string $field = null): array
    {
        ...
        $photoFields = ['voltage_ampere_photos', 'problem_photos'];
    ...
    public function render()
    {
        ...
        return view('livewire.shared.forms-dashboard.weekly-volt-ampere-dashboard', [
