...
class IncubatorRackPmDashboard extends Component
{
    public int $typeId;
    ...
    {
        $this->formType = FormType::where('form_name', 'Incubator Rack Preventive Maintenance Checklist')->where('isActive', true)->first();

        if (! $this->formType) {
            abort(404, 'Incubator Rack PM form type not found');
        }
    ...
    protected function baseQuery(): Builder
    {
        ...
        if ($search !== '') {
            $query->where(function ($q) use ($terms, $search) {
                ...
                })->orWhere('form_inputs', 'like', '%"rack_number"%' . $search . '%');
            });
        }
    ...
    public function render()
    {
        ...
        return view('livewire.shared.forms-dashboard.incubator-rack-pm-dashboard', [
