            ...
            <div class="relative w-full max-w-md p-6 bg-white dark:bg-gray-800 shadow-xl dark:shadow-2xl rounded-lg">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">
                        {{ !$isActive ? 'Activate GetSet' : 'Deactivate GetSet' }}
                    </h3>
                ...
                <div class="flex items-center mb-4">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-900 dark:text-white">
                            {{ !$isActive ? 'Activate GetSet' : 'Deactivate GetSet' }} <span class="font-semibold">{{ $getSetName }}</span>?
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            @if(!$isActive)
                                This will make the GetSet available for use again.
                            @else
                                This will make the GetSet unavailable for use. All data will be preserved.
                            @endif
                        </p>
                    </div>
                </div>
