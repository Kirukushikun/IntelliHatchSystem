<div>
    <!-- Header with Title, Search, and Add GetSet -->
    <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between md:gap-6">
        <div class="text-center md:text-left">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">GetSet Management</h1>
            <p class="text-gray-600 dark:text-gray-400">Manage generator sets (GetSets).</p>
        </div>
        <div class="flex flex-col gap-3 md:flex-row md:gap-3 md:items-center">
            <div class="flex flex-row gap-3 items-center w-full md:w-auto">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2">
                                        <label class="flex items-center">
                                            ...
                                            <span class="text-sm text-gray-700 dark:text-gray-300">All GetSets</span>
                                        </label>
                                        <label class="flex items-center">
                ...
                <button type="button" wire:click="$dispatch('openCreateModal')" class="inline-flex items-center justify-center px-4 py-3 text-sm font-medium text-white bg-orange-600 border border-orange-600 rounded-lg hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all duration-150 whitespace-nowrap shrink-0 md:px-4">
                    ...
                    <span class="hidden md:inline">Add GetSet</span>
                </button>
                @endif
            </div>
        </div>
    ...
    <div class="relative flex flex-col w-full h-full text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 shadow-md dark:shadow-lg rounded-lg bg-clip-border">
        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-left table-auto min-w-max">
                <thead>
                    <tr>
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-gray-600" wire:click="sortBy('getSetName')">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200 flex items-center gap-1">
                                GetSet Name
                                @if ($sortField === 'getSetName')
                    ...
                    @forelse ($getSets as $getSet)
                        <tr class="even:bg-slate-50 dark:even:bg-gray-700/50 hover:bg-slate-100 dark:hover:bg-gray-700">
                            <td class="p-3 md:p-4 py-4 md:py-5">
                                <p class="block text-xs md:text-sm text-slate-800 dark:text-slate-200">{{ $getSet->getSetName }}</p>
                            </td>
                            <td class="p-3 md:p-4 py-4 md:py-5">
                                <p class="block text-xs md:text-sm text-slate-800 dark:text-slate-200">{{ $getSet->creationDate->format('d M, Y') }}</p>
                            </td>
                            ...
                            <td class="p-3 md:p-4 py-4 md:py-5">
                                <div class="flex gap-1 md:gap-2 justify-center items-center">
                                    @if(auth()->check() && auth()->user()->user_type === 0)
                                        <button
                                            wire:click="$dispatch('openEditModal', '{{ $getSet->id }}')"
                                    ...
                                    @endif
                                    <button
                                        wire:click="$dispatch('openDisableModal', '{{ $getSet->id }}')"
                                        class="px-3 py-1 text-xs font-medium {{ $getSet->isActive ? 'text-red-600 bg-red-50 hover:bg-red-100' : 'text-green-600 bg-green-50 hover:bg-green-100' }} rounded-md transition-colors cursor-pointer"
                                        title="{{ $getSet->isActive ? 'Deactivate GetSet' : 'Activate GetSet' }}">
                                        {{ $getSet->isActive ? 'Deactivate' : 'Activate' }}
                                    </button>
                                    @if(auth()->check() && auth()->user()->user_type === 0)
                                        <button
                                            wire:click="$dispatch('openDeleteModal', '{{ $getSet->id }}')"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">No get-sets found</h3>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="md:hidden space-y-4 p-4">
            @forelse ($getSets as $getSet)
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm dark:shadow-md p-4 space-y-3">
                    <div class="flex justify-between items-start">
                        <div class="space-y-1">
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ $getSet->getSetName }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $getSet->creationDate->format('d M, Y') }}</p>
                        </div>
                    ...
                    <div class="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <button
                            wire:click="$dispatch('openDisableModal', '{{ $getSet->id }}')"
                            class="px-3 py-1 text-xs font-medium {{ $getSet->isActive ? 'text-red-600 bg-red-50 hover:bg-red-100' : 'text-green-600 bg-green-50 hover:bg-green-100' }} rounded-md transition-colors"
                            title="{{ $getSet->isActive ? 'Deactivate GetSet' : 'Activate GetSet' }}">
                            {{ $getSet->isActive ? 'Deactivate' : 'Activate' }}
                        </button>
                        @if(auth()->check() && auth()->user()->user_type === 0)
            ...
            @empty
                <div class="flex flex-col items-center py-12">
                    ...
                    <h3 class="text-lg font-medium text-gray-900">No get-sets found</h3>
                </div>
            @endforelse
        </div>
        ...
        @if ($getSets->hasPages())
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center px-3 md:px-4 py-3 border-t border-slate-200 dark:border-gray-700 gap-3 sm:gap-0">
                <div class="text-xs md:text-sm text-slate-500 dark:text-slate-400 text-center sm:text-left">
                    Showing <b>{{ $getSets->firstItem() }}-{{ $getSets->lastItem() }}</b> of {{ $getSets->total() }}
                </div>
                <x-custom-pagination
    ...
    <livewire:shared.management.get-set-management.create on:getSetCreated="$refresh" />

    <!-- Include Edit Modal -->
    <livewire:shared.management.get-set-management.edit on:getSetUpdated="$refresh" />

    <!-- Include Delete Modal -->
    <livewire:shared.management.get-set-management.delete on:getSetDeleted="$refresh" />

    <!-- Include Disable/Enable Modal -->
    <livewire:shared.management.get-set-management.disable on:statusToggled="$refresh" />
</div>
