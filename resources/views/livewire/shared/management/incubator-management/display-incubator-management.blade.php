<div>
    ...
    <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between md:gap-6">
        <div class="text-center md:text-left">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Incubator Machines Management</h1>
            <p class="text-gray-600 dark:text-gray-400">Manage incubators.</p>
        </div>
        <div class="flex flex-col gap-3 md:flex-row md:gap-3 md:items-center">
            <div class="flex flex-row gap-3 items-center w-full md:w-auto">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2">
                                        <label class="flex items-center">
                                            ...
                                            <span class="text-sm text-gray-700 dark:text-gray-300">All Incubators</span>
                                        </label>
                                        <label class="flex items-center">
                ...
                <button type="button" wire:click="$dispatch('openCreateModal')" class="inline-flex items-center justify-center px-4 py-3 text-sm font-medium text-white bg-orange-600 border border-orange-600 rounded-lg hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all duration-150 whitespace-nowrap shrink-0 md:px-4">
                    ...
                    <span class="hidden md:inline">Add Incubator</span>
                </button>
                @endif
            </div>
        </div>
    ...
    <div class="relative flex flex-col w-full h-full text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 shadow-md dark:shadow-lg rounded-lg bg-clip-border">
        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-left table-auto min-w-max">
                <thead>
                    <tr>
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-gray-600" wire:click="sortBy('incubatorName')">
                            ...
                            <td class="p-3 md:p-4 py-4 md:py-5">
                                <div class="flex gap-1 md:gap-2 justify-center items-center">
                                    ...
                                    <button 
                                        ...
                                        class="px-3 py-1 text-xs font-medium {{ $incubator->isActive ? 'text-red-600 bg-red-50 hover:bg-red-100' : 'text-green-600 bg-green-50 hover:bg-green-100' }} rounded-md transition-colors cursor-pointer"
                                        title="{{ $incubator->isActive ? 'Deactivate Incubator' : 'Activate Incubator' }}">
                                        {{ $incubator->isActive ? 'Deactivate' : 'Activate' }}
                                    </button>
                                    @if(auth()->check() && auth()->user()->user_type === 0)
                                        <button 
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">No incubators found</h3>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            @forelse ($incubators as $incubator)
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm dark:shadow-md p-4 space-y-3">
                    ...
                    <div class="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <button 
                            wire:click="$dispatch('openDisableModal', '{{ $incubator->id }}')"
                            class="px-3 py-1 text-xs font-medium {{ $incubator->isActive ? 'text-red-600 bg-red-50 hover:bg-red-100' : 'text-green-600 bg-green-50 hover:bg-green-100' }} rounded-md transition-colors"
                            title="{{ $incubator->isActive ? 'Deactivate Incubator' : 'Activate Incubator' }}">
                            {{ $incubator->isActive ? 'Deactivate' : 'Activate' }}
                        </button>
                        @if(auth()->check() && auth()->user()->user_type === 0)
