        ...
        <div x-show="showModal"
            ...
            <div class="bg-white dark:bg-gray-800 px-6 py-3 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">WEEKLY VOLTAGE AND AMPERE MONITORING</h3>
            ...
            <div class="bg-white dark:bg-gray-800 px-6 py-4 max-h-[70vh] overflow-y-auto">
                @if($this->selectedForm)
                    @php
                        ...
                        if (isset($this->formData['maintenance_personnel']) && is_numeric($this->formData['maintenance_personnel'])) {
                            $personnelUser = \Illuminate\Support\Facades\DB::table('users')->where('id', $this->formData['maintenance_personnel'])->first();
                    ...
                    <div class="space-y-4 mb-6">
                        ...
                        <div class="flex justify-between items-center py-3 border-b border-gray-100 dark:border-gray-700"><span class="text-sm font-medium text-gray-600 dark:text-gray-400">Maintenance Personnel:</span><span class="text-sm text-gray-900 dark:text-gray-200">{{ $personnelDisplay }}</span></div>
                        <div class="flex justify-between items-center py-3 border-b border-gray-100 dark:border-gray-700"><span class="text-sm font-medium text-gray-600 dark:text-gray-400">Date:</span><span class="text-sm text-gray-900 dark:text-gray-200">{{ $this->formData['date'] ?? 'N/A' }}</span></div>
                        <div class="flex justify-between items-center py-3 border-b border-gray-100 dark:border-gray-700"><span class="text-sm font-medium text-gray-600 dark:text-gray-400">Time Started:</span><span class="text-sm text-gray-900 dark:text-gray-200">{{ $this->formData['time_started'] ?? 'N/A' }}</span></div>
                    ...
                    <div class="mb-6">
                        <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-4">Readings</h4>
                        <div class="space-y-4">
                            <div class="py-3 border-b border-gray-100 dark:border-gray-700">
                                <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Voltage Readings:</span>
                                <p class="text-sm text-gray-900 dark:text-gray-200 mt-1 whitespace-pre-wrap">{{ $this->formData['voltage_readings'] ?? 'N/A' }}</p>
                            </div>
                            <div class="py-3 border-b border-gray-100 dark:border-gray-700">
                                <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Ampere Readings:</span>
                                <p class="text-sm text-gray-900 dark:text-gray-200 mt-1 whitespace-pre-wrap">{{ $this->formData['ampere_readings'] ?? 'N/A' }}</p>
                            </div>
                            <div class="py-3 border-b border-gray-100 dark:border-gray-700">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Voltage/Ampere Photos:</span>
                                    @php $photoCount = $this->getPhotoCount('voltage_ampere_photos'); @endphp
                                    @if($photoCount > 0)
                                        <button @click="$wire.viewPhotos('voltage_ampere_photos')" class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold rounded-lg border border-blue-200 dark:border-blue-600 bg-blue-50 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 hover:bg-blue-100 transition-all cursor-pointer">Photos ({{ $photoCount }})</button>
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-4">Problem & Corrective Action</h4>
                        <div class="space-y-4">
                            <div class="py-3 border-b border-gray-100 dark:border-gray-700">
                                <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Problem / Corrective Action:</span>
                                <p class="text-sm text-gray-900 dark:text-gray-200 mt-1 whitespace-pre-wrap">{{ $this->formData['problem_corrective_action'] ?? 'N/A' }}</p>
                            </div>
                            <div class="py-3 border-b border-gray-100 dark:border-gray-700">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-sm font-medium text-gray-600 dark:text-gray-400">Problem Photos:</span>
                                    @php $photoCount = $this->getPhotoCount('problem_photos'); @endphp
                                    @if($photoCount > 0)
                                        <button @click="$wire.viewPhotos('problem_photos')" class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold rounded-lg border border-blue-200 dark:border-blue-600 bg-blue-50 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 hover:bg-blue-100 transition-all cursor-pointer">Photos ({{ $photoCount }})</button>
                                    @elseif($this->selectedForm && $this->selectedForm->photos_purged_at)
                                        <span class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-600">Photos Expired</span>
                                    @else
                                        <span class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-600">No Photos</span>
