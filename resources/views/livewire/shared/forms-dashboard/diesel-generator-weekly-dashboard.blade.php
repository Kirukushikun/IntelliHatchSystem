<div wire:poll.60s.visible wire:key="{{ now()->timestamp }}">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-6">
        <div class="text-center sm:text-left">
            ...
            <p class="text-gray-600 dark:text-gray-400">All submitted diesel generator weekly maintenance forms</p>
        </div>
        <div class="relative w-full sm:w-auto sm:shrink-0">
            ...
            <input
                ...
                placeholder="Search by technician, gen set..."
                    ...
                    <div class="p-4">
                        ...
                        <div class="space-y-2">
                            <div>
                                ...
                                <input type="date" wire:model="dateFrom" class="w-full px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent" max="{{ $dateTo ?: now()->format('Y-m-d') }}" />
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">To</label>
                                <input type="date" wire:model="dateTo" class="w-full px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent" max="{{ now()->format('Y-m-d') }}" min="{{ $dateFrom ?: '' }}" />
                            </div>
                        </div>
    ...
    <div class="relative flex flex-col w-full h-full text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 shadow-md dark:shadow-lg rounded-lg bg-clip-border">
                ...
                <thead>
                    <tr>
                        ...
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200">Technician</p>
                        </th>
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200">Gen Set</p>
                        </th>
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200">Tank Level</p>
                        </th>
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200">Running Condition</p>
                        </th>
                ...
                <tbody>
                    @forelse ($forms as $form)
                        @php
                            ...
                            $techDisplay = $form->user ? trim($form->user->first_name . ' ' . $form->user->last_name) : 'Unknown';
                            $genSetName = $formData['machine_info']['name'] ?? 'N/A';
                        @endphp
                        <tr class="even:bg-slate-50 dark:even:bg-gray-700/50 hover:bg-slate-100 dark:hover:bg-gray-700">
                            ...
                            <td class="p-3 md:p-4 py-4 md:py-5 text-left">
                                <p class="block text-xs md:text-sm text-slate-800 dark:text-slate-200">{{ $techDisplay }}</p>
                            </td>
                            <td class="p-3 md:p-4 py-4 md:py-5 text-left">
                                <p class="block text-xs md:text-sm text-slate-800 dark:text-slate-200">{{ $genSetName }}</p>
                            </td>
                            <td class="p-3 md:p-4 py-4 md:py-5 text-left">
                                @php $tankLevel = $formData['diesel_tank_level'] ?? 'N/A'; @endphp
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $tankLevel === 'Full Tank' ? 'bg-green-100 text-green-800' : ($tankLevel === 'Half Tank' ? 'bg-yellow-100 text-yellow-800' : ($tankLevel === 'For Refill' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800')) }}">{{ $tankLevel }}</span>
                            </td>
                            <td class="p-3 md:p-4 py-4 md:py-5 text-left">
                                @php $condition = $formData['running_condition'] ?? 'N/A'; @endphp
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $condition === 'Normal' ? 'bg-green-100 text-green-800' : ($condition === 'Abnormal' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800') }}">{{ $condition }}</span>
                            </td>
        ...
        <div class="md:hidden space-y-4 p-4">
            @forelse ($forms as $form)
                @php
                    $formData = is_array($form->form_inputs) ? $form->form_inputs : [];
                    $mTechDisplay = $form->user ? trim($form->user->first_name . ' ' . $form->user->last_name) : 'Unknown';
                    $mGenSetName = $formData['machine_info']['name'] ?? 'N/A';
                @endphp
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm dark:shadow-lg p-4 space-y-3">
                    <div class="flex justify-between items-start">
                        <div class="space-y-1">
                            ...
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $mTechDisplay }}</p>
                        </div>
                    </div>
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Gen Set:</span>
                            <span class="text-xs text-gray-900 dark:text-gray-200">{{ $mGenSetName }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Tank Level:</span>
                            <span class="text-xs text-gray-900 dark:text-gray-200">{{ $formData['diesel_tank_level'] ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Condition:</span>
                            <span class="text-xs text-gray-900 dark:text-gray-200">{{ $formData['running_condition'] ?? 'N/A' }}</span>
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        <button wire:click="viewDetails({{ $form->id }})" class="px-3 py-1 text-xs font-medium text-blue-600 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/50 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/70 transition-colors" title="View Details">View</button>
                        <button wire:click="deleteForm({{ $form->id }})" class="px-3 py-1 text-xs font-medium text-red-600 dark:text-red-300 bg-red-50 dark:bg-red-900/40 rounded-md hover:bg-red-100 dark:hover:bg-red-900/60 transition-colors" title="Delete Form">Delete</button>
                    </div>
                </div>
            @empty
    ...
    <!-- Form Details Modal -->
    @include('livewire.shared.forms-dashboard.modals.diesel-generator-weekly-view')
            ...
            <div class="relative w-full max-w-md p-6 bg-white dark:bg-gray-800 shadow-xl dark:shadow-2xl rounded-lg">
                ...
                <div class="flex items-center mb-4">
                    <div class="shrink-0 w-12 h-12 bg-red-100 rounded-full flex items-center justify-center">
                        <svg class="w-6 h-6 text-red-600 dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                    </div>
                    <div class="ml-4">
