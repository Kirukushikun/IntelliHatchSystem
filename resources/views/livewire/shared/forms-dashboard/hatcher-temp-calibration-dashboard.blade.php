<div wire:key="{{ now()->timestamp }}">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-6">
        <div class="text-center sm:text-left">
            ...
            <p class="text-gray-600 dark:text-gray-400">All submitted forms of this type</p>
        </div>
        <div class="relative w-full sm:w-auto sm:shrink-0">
            <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            <input wire:model.live="search" placeholder="Search forms..." class="w-full pl-11 pr-20 py-3 text-sm bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent transition-all placeholder:text-gray-400 dark:placeholder:text-gray-500 shadow-sm" />
            <a href="{{ \Illuminate\Support\Facades\URL::temporarySignedRoute('admin.print.forms.hatcher-temp-calibration', now()->addMinutes(10), ['search' => $search, 'dateFrom' => $dateFrom, 'dateTo' => $dateTo, 'shiftFilter' => $shiftFilter, 'sortField' => $sortField, 'sortDirection' => $sortDirection]) }}" target="_blank" rel="noopener" class="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors cursor-pointer" title="Print / Save as PDF">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 9V2h12v7M6 18H4a2 2 0 01-2-2v-5a2 2 0 012-2h16a2 2 0 012 2v5a2 2 0 01-2 2h-2M6 14h12v8H6v-8z" /></svg>
            </a>
            <button type="button" wire:click="toggleFilterDropdown" class="absolute right-11 top-1/2 -translate-y-1/2 p-2 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors cursor-pointer">
                <svg width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" fill="#9CA3AF" class="w-5 h-5"><path fill-rule="evenodd" clip-rule="evenodd" d="M15 2v1.67l-5 4.759V14H6V8.429l-5-4.76V2h14zM7 8v5h2V8l5-4.76V3H2v.24L7 8z"/></svg>
            </button>
                ...
                <div class="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50">
                    <div class="p-4">
                        <div class="mb-4">
                            ...
                            <div class="space-y-2">
                                <div><label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">From</label><input type="date" wire:model="dateFrom" class="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent" max="{{ $dateTo ?: now()->format('Y-m-d') }}" wire:loading.attr="disabled" /></div>
                                <div><label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">To</label><input type="date" wire:model="dateTo" class="w-full px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent" max="{{ now()->format('Y-m-d') }}" min="{{ $dateFrom ?: '' }}" wire:loading.attr="disabled" /></div>
                            </div>
                        </div>
                        <div>
    ...
    <div class="flex flex-wrap gap-3 mb-6">
        ...
        <button wire:click="quickFilterToday" class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors {{ ($dateFrom === now()->format('Y-m-d') && $dateTo === now()->format('Y-m-d') && $shiftFilter === 'all') ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-800' }}">
            Today <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ ($dateFrom === now()->format('Y-m-d') && $dateTo === now()->format('Y-m-d') && $shiftFilter === 'all') ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-200' }}">{{ $todayFormCount ?? 0 }}</span>
        </button>
        @foreach(['1st Shift', '2nd Shift', '3rd Shift'] as $shift)
            <button wire:click="quickFilterShift('{{ $shift }}')" class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors {{ ($shiftFilter === $shift && $dateFrom === now()->format('Y-m-d') && $dateTo === now()->format('Y-m-d')) ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-800' }}">
                {{ $shift }} <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ ($shiftFilter === $shift && $dateFrom === now()->format('Y-m-d') && $dateTo === now()->format('Y-m-d')) ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-200' }}">{{ $todayShiftCounts[$shift] ?? 0 }}</span>
            </button>
        @endforeach
    </div>

    <div wire:poll.60s.visible class="relative flex flex-col w-full h-full text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 shadow-md dark:shadow-lg rounded-lg bg-clip-border">
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-left table-auto min-w-max">
                <thead>
                    <tr>
                        <th class="p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-gray-700" wire:click="sortBy('date_submitted')">
                            <p class="text-sm font-semibold leading-none text-slate-700 dark:text-slate-200 flex items-center gap-1">Date Submitted @if ($sortField === 'date_submitted')<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">@if ($sortDirection === 'asc')<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path>@else<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>@endif</svg>@endif</p>
                        </th>
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="5" class="p-8 text-center"><div class="flex flex-col items-center justify-center"><svg class="w-12 h-12 text-slate-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><p class="text-sm text-slate-600 font-medium">No forms found</p></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
            ...
            @forelse($forms as $form)
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-4 space-y-3 mb-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex justify-between"><span class="text-xs font-medium text-gray-500 dark:text-gray-400">Hatcher:</span><span class="text-xs text-gray-900 dark:text-gray-200">{{ $formData['machine_info']['name'] ?? 'N/A' }}</span></div>
                    ...
                    <div class="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <button wire:click="deleteForm({{ $form->id }})" class="px-3 py-1 text-xs font-medium text-red-600 dark:text-red-300 bg-red-50 dark:bg-red-900/40 rounded-md hover:bg-red-100 transition-colors">Delete</button>
                    </div>
                </div>
            @empty
                <div class="flex flex-col items-center py-12"><svg class="w-12 h-12 text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg><p class="text-gray-500 dark:text-gray-400 mt-1">No forms found</p></div>
            @endforelse
        </div>
    ...
    @include('livewire.shared.forms-dashboard.modals.hatcher-temp-calibration-view')
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-9999 flex items-center justify-center p-4" wire:ignore.self>
            ...
            <div class="relative w-full max-w-md p-6 bg-white dark:bg-gray-800 shadow-xl rounded-lg">
                <div class="flex items-center justify-between mb-4"><h3 class="text-lg font-medium text-gray-900 dark:text-white">Delete Form</h3><button type="button" wire:click="cancelDelete" class="text-gray-400 hover:text-gray-600 cursor-pointer"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div>
                <div class="flex items-center mb-4"><div class="shrink-0 w-12 h-12 bg-red-100 rounded-full flex items-center justify-center"><svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg></div><div class="ml-4"><p class="text-sm font-medium text-gray-900 dark:text-white">Are you sure you want to delete this form?</p><p class="text-sm text-gray-500 dark:text-gray-400">This action cannot be undone.</p></div></div>
                <div class="flex justify-end gap-3"><x-button variant="outline-secondary" type="button" wire:click="cancelDelete" class="cursor-pointer">Cancel</x-button><x-button variant="danger" type="button" wire:click="confirmDelete" class="cursor-pointer">Delete Form</x-button></div>
            </div>
        </div>
    @endif
</div>
