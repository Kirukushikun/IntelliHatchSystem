<div wire:poll.60s.visible wire:key="{{ now()->timestamp }}">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-6">
        <div class="text-center sm:text-left">
            ...
            <p class="text-gray-600 dark:text-gray-400">All submitted incubator rack PM forms</p>
        </div>
        <div class="relative w-full sm:w-auto sm:shrink-0">
            ...
            <input
                ...
                placeholder="Search by personnel, rack number..."
    ...
    <div class="relative flex flex-col w-full h-full text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 shadow-md dark:shadow-lg rounded-lg bg-clip-border">
            ...
            <table class="w-full text-left table-auto min-w-max">
                <thead>
                    <tr>
                        ...
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200">Rack Number</p>
                        </th>
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200">Date</p>
                        </th>
                ...
                <tbody>
                    @forelse ($forms as $form)
                        @php
                            ...
                            $personnelDisplay = 'N/A';
                            if (isset($formData['maintenance_personnel']) && is_numeric($formData['maintenance_personnel'])) {
                                $personnelUser = \Illuminate\Support\Facades\DB::table('users')->where('id', $formData['maintenance_personnel'])->first();
                        ...
                        <tr class="even:bg-slate-50 dark:even:bg-gray-700/50 hover:bg-slate-100 dark:hover:bg-gray-700">
                            ...
                            <td class="p-3 md:p-4 py-4 md:py-5 text-left">
                                <p class="block text-xs md:text-sm text-slate-800 dark:text-slate-200">{{ $formData['rack_number'] ?? 'N/A' }}</p>
                            </td>
                            <td class="p-3 md:p-4 py-4 md:py-5 text-left">
                                <p class="block text-xs md:text-sm text-slate-800 dark:text-slate-200">{{ $formData['date'] ?? 'N/A' }}</p>
                            </td>
        ...
        <div class="md:hidden space-y-4 p-4">
            @forelse ($forms as $form)
                @php
                    $formData = is_array($form->form_inputs) ? $form->form_inputs : [];
                    $mPersonnelDisplay = 'N/A';
                    if (isset($formData['maintenance_personnel']) && is_numeric($formData['maintenance_personnel'])) {
                        $mPersonnelUser = \Illuminate\Support\Facades\DB::table('users')->where('id', $formData['maintenance_personnel'])->first();
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm dark:shadow-lg p-4 space-y-3">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Rack Number:</span>
                            <span class="text-xs text-gray-900 dark:text-gray-200">{{ $formData['rack_number'] ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Date:</span>
                            <span class="text-xs text-gray-900 dark:text-gray-200">{{ $formData['date'] ?? 'N/A' }}</span>
                        </div>
                    </div>
    ...
    <!-- Form Details Modal -->
    @include('livewire.shared.forms-dashboard.modals.incubator-rack-pm-view')
