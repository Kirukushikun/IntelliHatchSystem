<div>
    ...
    @if ($step === 1)
        <div class="flex flex-col gap-4 mb-6">
            <div class="text-center md:text-left">
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Import Incubator Temperature Calibration</h1>
                <p class="text-gray-600 dark:text-gray-400">Upload a CSV file to bulk import Incubator Temperature Calibration submissions</p>
            </div>
        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                        ...
                        <ul class="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-disc list-inside">
                            <li>Required columns: <strong>Hatchery Man</strong>, <strong>Date Submitted</strong>, <strong>Shift</strong>, <strong>Time Started</strong>, <strong>Time finished</strong></li>
                            <li>Incubator columns: Incubator 1-10 Machine Temp / Calibrator Temp readings and Humidity readings</li>
                            <li>Temp formats supported: "99.2 / 99.2", "Temp reading 99.3/ calibrator 99.6", etc.</li>
                            <li>Incubators with "0" or empty values are skipped (no reading taken)</li>
                        </ul>
                    </div>
                    ...
                    <div class="flex justify-end">
                        <button
                            type="submit"
                            class="px-6 py-3 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    @if ($step === 2)
        <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">
            <div class="text-center md:text-left">
                ...
                <p class="text-gray-600 dark:text-gray-400">Review parsed data and map hatcherymen to system users before importing</p>
            </div>
            <button
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4 text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Unique Hatcherymen</p>
                @if ($unmatchedCount > 0)
        ...
        <div class="bg-white dark:bg-gray-800 shadow-md dark:shadow-lg rounded-lg p-6 mb-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Hatcheryman to User Mapping</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Map each CSV hatcheryman name to an existing system user. All must be mapped to proceed.</p>
                ...
                @foreach ($csvNames as $index => $name)
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 p-3 rounded-lg {{ empty($userMapping[$index]) ? 'bg-orange-50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-800' : 'bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800' }}">
                        ...
                        <div class="flex items-center gap-2">
                            <svg class="w-5 h-5 text-gray-400 dark:text-gray-500 hidden sm:block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/>
                            </svg>
                            <select
                                wire:model.live="userMapping.{{ $index }}"
                                class="w-full sm:w-64 px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent dark:bg-gray-700 dark:text-white"
                            >
                                ...
                                <td class="p-3 text-xs text-slate-800 dark:text-slate-200">
                                    @foreach ($row['incubators'] as $h)
                                        <span class="inline-block bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded px-1.5 py-0.5 text-xs mr-1 mb-1">
                                            I{{ $h['incubator_num'] }}: {{ $h['machine_temp'] }}/{{ $h['calibrator_temp'] }}
                                        </span>
                                    @endforeach
                                </td>
        ...
        <div class="flex justify-end gap-3">
            <button
                wire:click="resetImport"
                class="px-6 py-3 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer"
            >
                Cancel
            </button>
            <button
                wire:click="executeImport"
                wire:confirm="Are you sure you want to import {{ number_format($totalFormRecords) }} form records? This action cannot be undone."
                class="px-6 py-3 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-lg transition-colors shadow-sm cursor-pointer disabled:opacity-50"
                wire:loading.attr="disabled"
            >
    ...
    @if ($step === 3)
                ...
                <div class="max-h-40 overflow-y-auto">
                    <ul class="text-xs text-red-700 dark:text-red-400 space-y-1 list-disc list-inside">
                        ...
                        @if (count($importErrors) > 50)
                            <li class="font-medium">... and {{ count($importErrors) - 50 }} more</li>
                        @endif
                    </ul>
                </div>
            </div>
        ...
        <div class="flex justify-end gap-3">
            <button
                wire:click="resetImport"
                class="px-6 py-3 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-sm cursor-pointer"
            >
                Import Another File
            </button>
        </div>
