<div>
    <!-- Header with Title, Search, and Add Account -->
    <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between md:gap-6">
        <div class="text-center md:text-left">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Admin Management</h1>
            <p class="text-gray-600 dark:text-gray-400">Manage admin and superadmin accounts</p>
        </div>
        <div class="flex flex-col gap-3 md:flex-row md:gap-3 md:items-center">
            <div class="flex flex-row gap-3 items-center w-full md:w-auto">
                        ...
                        <div class="absolute top-full mt-2 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-xl border border-gray-200 dark:border-gray-700 z-50 left-0 right-0 md:left-auto md:right-0 md:w-80">
                            <div class="p-4">
                                <div class="grid grid-cols-2 gap-1">
                                    <!-- Role Filter Column -->
                                    <div>
                                        <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Role</h3>
                                        <div class="space-y-2">
                                            <label class="flex items-center">
                                                <input type="radio" wire:model="roleFilter" value="all" class="mr-2">
                                                <span class="text-sm text-gray-700 dark:text-gray-300">All Roles</span>
                                            </label>
                                            <label class="flex items-center">
                                                <input type="radio" wire:model="roleFilter" value="superadmin" class="mr-2">
                                                <span class="text-sm text-gray-700 dark:text-gray-300">Superadmin</span>
                                            </label>
                                            <label class="flex items-center">
                                                <input type="radio" wire:model="roleFilter" value="admin" class="mr-2">
                                                <span class="text-sm text-gray-700 dark:text-gray-300">Admin</span>
                                            </label>
                                        </div>
                                        <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3 mt-3">Status</h3>
                                        <div class="space-y-2">
                                            <label class="flex items-center">
                ...
                <button type="button" wire:click="$dispatch('openCreateAdminModal')" class="inline-flex items-center justify-center px-4 py-3 text-sm font-medium text-white bg-orange-600 border border-orange-600 rounded-lg hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all duration-150 whitespace-nowrap shrink-0 md:px-4 cursor-pointer">
            ...
            <table class="w-full text-left table-auto min-w-max">
                <thead>
                    <tr>
                        ...
                        <th class="p-3 md:p-4 border-b border-slate-300 dark:border-gray-600 bg-slate-50 dark:bg-gray-700 text-center">
                            <p class="text-xs md:text-sm font-semibold leading-none text-slate-700 dark:text-slate-200">Role</p>
                        </th>
                ...
                <tbody>
                    @forelse ($users as $user)
                        @php
                            $isProtected = $user->id === $firstSuperadminId && $currentUserId !== $firstSuperadminId;
                        @endphp
                            ...
                            <td class="p-3 md:p-4 py-4 md:py-5">
                                ...
                                @else
                                    <div class="flex gap-1 md:gap-2 justify-center">
                                        <button
                                            wire:click="$dispatch('openEditAdminModal', '{{ $user->id }}')"
                                        ...
                                        <button
                                            wire:click="$dispatch('openChangePasswordAdminModal', '{{ $user->id }}')"
                                            class="px-3 py-1 text-xs font-medium text-yellow-600 bg-yellow-50 rounded-md hover:bg-yellow-100 transition-colors cursor-pointer"
                                            title="Change Password">
                                            Password
                                        </button>
                                        <button
                                            wire:click="$dispatch('openDisableAdminModal', '{{ $user->id }}')"
                                            class="px-3 py-1 text-xs font-medium {{ $user->is_disabled ? 'text-green-600 bg-green-50 hover:bg-green-100' : 'text-red-600 bg-red-50 hover:bg-red-100' }} rounded-md transition-colors cursor-pointer"
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">No accounts found</h3>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            @forelse ($users as $user)
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm dark:shadow-md p-4 space-y-3">
                    ...
                    <div class="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        @if ($isProtected)
                            <span class="text-xs text-gray-400 dark:text-gray-500 italic">Protected account</span>
                        @else
                            <button
                                wire:click="$dispatch('openEditAdminModal', '{{ $user->id }}')"
    ...
    <!-- Modals -->
    <livewire:admin.admin-management.create on:refreshAdmins="$refresh" />
    <livewire:admin.admin-management.edit on:refreshAdmins="$refresh" />
    <livewire:admin.admin-management.delete on:refreshAdmins="$refresh" />
    <livewire:admin.admin-management.disable on:refreshAdmins="$refresh" />
    <livewire:admin.admin-management.change-password />
</div>
